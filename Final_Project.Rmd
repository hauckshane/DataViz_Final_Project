...
ui <- fluidPage(
  ...
  mainPanel(
    ...
    prettyCheckbox("checkRank", "rank", FALSE, inline = TRUE),
    tabsetPanel(
      tabPanel("Shooting", 
...
##SHOOTING
  ...
  hc_add_series(data = if(input$checkRank == FALSE) {epl_shoot_full_scaled()}
                        else{epl_shooting_rank_final()}, 
                mapping = hcaes(x = variable, y = if(input$checkRank == FALSE) {value.scaled}
                                                    else{rank.scaled}), 
                type = "column",
                tooltip = list(headFormat = "{point.variable}", pointFormat = "{point.value}"),
                showInLegend = FALSE,
  ...
  epl_shooting_rank <- reactive({
    if(input$checkPOS == FALSE) {
      ...
      player_pos <- left_join(epl_standard, epl_positions) %>% 
        ...
        pivot_longer(cols = c(Pos1, Pos2, Pos3), names_to = "type", values_to = "position") %>%
        filter(!is.na(position))
      semi_join(if(input$check90 == FALSE){epl_shooting}
                else{epl_shootin_90}, player_pos, by = c("position" = "position")) %>%
      pivot_wider(names_from = "type", values_from = "position") %>%
      filter(`90s` >= 4) %>%
      drop_na() %>%
      mutate(across(-c(Age, Squad, Pos, Nation, Name, Rk, Born, `90s`, FK, PKatt, PK), min_rank, .names = "rank_{.col}")) %>%
...
## Shooting 90
  ...
  hc_add_series(data = shooting_descr,
                ...
                animation = FALSE,
                showInLegend = FALSE) %>%   
  hc_add_series(data = if(input$checkRank == FALSE){epl_shoot_90_full_scaled()}
                        else{epl_shooting_rank_final}, 
                mapping = hcaes(x = variable, y = if(input$checkRank == FALSE){value.scaled}
                                                    else{rank.scaled}), 
                type = "column",
