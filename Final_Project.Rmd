...
datatable(data, options = list(pageLength = 25,
      ...
      columnDefs = list(list(className = 'dt-center', targets = "_all"),
                      list(targets = 4, visible = F)))) %>% formatStyle(c("Stat", "Value", "Scaled Value"), "row.color", backgroundColor = styleEqual(sort(unique(data$row.color)), sort(unique(data$row.color)))) %>%
...
ui <- fluidPage(
  ...
  mainPanel(
    ...
    prettyCheckbox("checkColAr", "area", FALSE, inline = TRUE),
...
server <- function(input, output, session) {
  ...
  output$shootTable <- renderTable({
    if (input$check90 == FALSE) {
      epl_shooting_rank_final()
      ##epl_shoot_table()
    }
    else {epl_shooting_rank_final()##epl_shoot_90_table()
      }
  })
